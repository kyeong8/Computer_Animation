...
init(const char* filename)
{
	...
	readMesh(filename, vertex, face, faceNormal, vertexNormal, edge);
...
drawShrunkenFacesonWireFrame()
{
	...
	for (int i = 0; i < vertex.cols(); i++)
	{
		...
		{
			glVertex3f(vertex(0, i), vertex(1, i), vertex(2, i));
			glVertex3f(vertex(0, edge[i][j]), vertex(1, edge[i][j]), vertex(2, edge[i][j]));
		}
	}
	glEnd();

	glLineWidth(2 * dpiScaling);
	setupColoredMaterial(Vector3f(0.9f, 0.9f, 0.9f));
	...
	// Material
	setupColoredMaterial(Vector3f(0.033f, 1.0f, 1.0f));
	glBegin(GL_TRIANGLES);
	for (int i = 0; i < face.cols(); i++)
	{
		if (useFaceNormal)	glNormal3fv(faceNormal.col(i).data());
...
render(GLFWwindow* window)
{
	...
	if (faceWithGapMesh)	drawShrunkenFacesonWireFrame();
	else                    drawMesh();
}
