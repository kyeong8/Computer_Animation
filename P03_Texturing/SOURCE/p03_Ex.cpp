...
bool
loadColorAlphaTexture(const char* filenameC, const char* filenameA, int w, int h)
{
	// Open the raw texture file
	ifstream	is(filenameC, ios::binary);
	if (is.fail())
	{
		cout << "Can't open " << filenameC << endl;
		return	false;
	}
	GLubyte* rawRGB = new GLubyte[w * h * 3];
	is.read((char*)rawRGB, w * h * 3);

	// Open the raw texture file
	ifstream	iss(filenameA, ios::binary);
	if (iss.fail())
	{
	...
	{
		if (!is)	cout << "Error: only " << is.gcount() << "bytes could be read!" << endl;
		if (!iss)	cout << "Error: only " << iss.gcount() << "bytes could be read!" << endl;

		for (int i = 0; i < w * h; i++)
		{
			raw[4 * i + 0] = rawRGB[3 * i + 0];			// R
			raw[4 * i + 1] = rawRGB[3 * i + 1];			// G
			raw[4 * i + 2] = rawRGB[3 * i + 2];			// B
			raw[4 * i + 3] = rawG[i];	// A
		}
...
void
renderColorAlphaNormalMappedQuad(const Vector3f& l)
{
	// Model  matrix
	Affine3f	T;	T = Translation3f(0.0f, 0.0f, 0.2f) * Scaling(0.7f, 0.7f, 0.7f);
	...
	// Texture
	setUniformi(pgNormalMapping.pg, "texDiffuse", 0);	// 4 for GL_TEXTURE4
