...
selectObjects(GLFWwindow* window, double x, double y)
{
	...
	GLuint	selectBuffer[64 * 5];
	glSelectBuffer(64 * 5, selectBuffer);
...
void
select(GLFWwindow* window, double x, double y)
{
	...
	if (picked != -1)
	{
		switch (pickMode)
		{
		case VERTEX:	Dv[picked] = 0; pickedNeighborVertices.insert({ picked, 0 }); break;
		case EDGE:		De[picked] = 0; pickedNeighborEdges.insert({ picked, 0 }); break;
		case FACE:		Df[picked] = 0; pickedNeighborFaces.insert({ picked, 0 }); break;
		case NONE:		break;
		}
		...
		if (nRing >= 1)
		{
			switch (pickMode)
			{
			case VERTEX:	findNeighborVertices(picked, nRing, pickedNeighborVertices); break;
			case EDGE:		findNeighborEdges(picked, nRing, pickedNeighborEdges); break;
			case FACE:		findNeighborFaces(picked, nRing, pickedNeighborFaces); break;
			case NONE:		break;
			}
		}
	}
