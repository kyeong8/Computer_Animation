...
main(void)
{
	...
	float	angle = twisting * length(newVertexPosition.xy);
	float	cosLength = cos(angle);
	float	sinLength = sin(angle);

	// New position due to twisting
	float	x = cosLength * newVertexPosition.x - sinLength * newVertexPosition.y;
	float	y = sinLength * newVertexPosition.x + cosLength * newVertexPosition.y;
	newVertexPosition = vec4(x, y, newVertexPosition.z, 1.0);
...
//	x = cosLength * newVertexNormal.x - sinLength * newVertexNormal.y;
//	y = sinLength * newVertexNormal.x + cosLength * newVertexNormal.y;
//	newVertexNormal = vec3(x, y, newVertexNormal.z);
