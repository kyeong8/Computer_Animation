...
Program pgWavenTwist;	// Program for the twist and wave deformer
...
main(int argc, char* argv[])
{
	...
	// Initialization
	{
		...
		pgWavenTwist.create("sv04_waventwist.glsl", "sf02_Phong.glsl");
...
render(GLFWwindow* window)
{
	...
	else if (example == 2)
	{
		...
		// Model, view, projection matrices
		setUniformMVP(pgWavenTwist.pg, ModelMatrix, ViewMatrix, ProjectionMatrix);

		// Light position in the fragment shader, represented in the view coordinate system
		Vector3f	l = ViewMatrix.block<3, 3>(0, 0) * light2 + ViewMatrix.block<3, 1>(0, 3);
		setUniform(pgWavenTwist.pg, "LightPosition", l);

		// Draw objects: only the bunny in this case.
		{
			// Material in dependent of the object
			setUniform(pgWavenTwist.pg, "Ka", Vector3f(0.10f, 0.10f, 0.10f));
			setUniform(pgWavenTwist.pg, "Kd", Vector3f(0.75f, 0.75f, 0.75f));
			setUniform(pgWavenTwist.pg, "Ks", Vector3f(0.10f, 0.10f, 0.10f));
			setUniform(pgWavenTwist.pg, "Shininess", 128.0f);

			// Twisting
			setUniform(pgWavenTwist.pg, "twisting", tau);

			// Phase
			setUniform(pgWavenTwist.pg, "phase", 4 * tau);

			// Spatial frequency
			setUniform(pgWavenTwist.pg, "F", frequency);

			// Draw the mesh using the program and the vertex buffer object
			glUseProgram(pgWavenTwist.pg);
			drawVBO(plane[level].vao, plane[level].numTris);
		}
	}
...
keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		case GLFW_KEY_R:		CCW = !CCW;			break;		// Direction
			...
			// Examples
		case GLFW_KEY_T:	example = (example + 1) % 3;	break;
