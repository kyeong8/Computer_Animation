...
uniform vec3	Light0Position;					// In the view coordinate system
uniform vec3	Light1Position;					// In the view coordinate system
...
Light L0 = Light(
	Light0Position,			// Position in the eye space
...
Light L1 = Light(
	Light1Position,			// Position in the eye space
	vec3(1.0, 1.0, 1.0),	// Ambient 
	vec3(1.0, 1.0, 1.0),	// Diffuse
	vec3(1.0, 1.0, 1.0),	// Specular
	vec3(1.0, 0.0, 0.0)		// Attenuation (const, linear, quad)
...
main(void)
{
	...
	outColor += phongReflectionModel(L1, position, normal);
}
