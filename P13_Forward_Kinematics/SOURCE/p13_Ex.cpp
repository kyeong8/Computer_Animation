...
void
drawLink(float radius, float height, const Vector3f& color, bool flag)
{
	glPushMatrix();
...
drawBase(float radiusLink, const Vector3f& color)
{
	...
	drawLink(radiusLink, (baseLinkHeight + baseHeight / 2), color, true);
...
float jointAngle[nLinks] = { 0.0f, 0.0f, 0.0f }; // Radian
...
drawKinematicModelUsingOpenGL()
{
	...
	glPushMatrix();
	glTranslatef(0, 0, 2 * radiusJoint);
	drawAxes(0.3, 3);
	glPopMatrix();
	...
	for (int i = 0; i < nLinks; i++)
	{
		...
		drawLink(radiusLink, linkLength[i], Vector3f(0, 0.95f, 0.95f), false); // Draw a link

		// Link transform
		glTranslatef(linkLength[i], 0, 0);
		...
		glVertex3f(0, 0, 0); glVertex3f(0.3, 0, 0); // x-axis
		glVertex3f(0, 0, 0); glVertex3f(0, 0.3, 0); // y-axis
		glVertex3f(0, 0, 0); glVertex3f(0, 0, 0.3); // z-axis
		glEnd();
		glEnable(GL_LIGHTING);
...
drawKinematicModelUsingEigen()
{
	...
	for (int i = 0; i < nLinks; i++)
	{
		...
		T_link.setIdentity();
		T_link.block<3, 1>(0, 3) = Vector3f(linkLength[i], 0.0, 0.0);
		T = T * T_link;
...
keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		case GLFW_KEY_3:  selectedJoint = 2;   break;
			...
			//  Joint angles
		case GLFW_KEY_LEFT: jointAngle[selectedJoint] += 0.1f;  cout << 180.0 * jointAngle[selectedJoint] / M_PI << endl;  break;
		case GLFW_KEY_RIGHT:jointAngle[selectedJoint] -= 0.1f;  cout << 180.0 * jointAngle[selectedJoint] / M_PI << endl;  break;
